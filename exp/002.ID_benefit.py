'''之前的单客户 XNN 脱敏回流，训练数据和测试数据的人是不相交的，
因此验证的收益仅为客户场景风格带来的 overfit 收益。
本实验将测试数据中的人选择为回流的人的子集，看看客户 ID overfit 的收益。'''
...
class xnn_parts:
    ...
    def tail(): # 注意用的 layer_num=4，并不是最后 3 层
        return block.model.mobile_face_net.MobileFaceNetTail(layer_num=4)
...
def main():
    ...
    work_dir = f'{result_path}/{dataset_name}'
    # 配置数据集
