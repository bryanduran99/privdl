'''https://fr-discourse.megvii-inc.com/t/topic/11587/51
看看中间层矩阵混淆的 tradeoff 曲线'''
...
def split_dataset(dataset, img_per_id=50):
    ...
    trainset, testset_by_person = hubble_utils.split_dataset_by_person(
        dataset, test_id_num=1000, test_img_per_id=img_per_id)
...
def main():
    ...
    obfuscation_type = utils.input_option(
        'obfuscation_type(default: mid_conv)', ['no', 'mat', 'mat_full', 'img_conv', 'mid_conv', 'mid_mat'])
    ...
    if obfuscation_type == 'no':
        ...
        mid_layers = utils.input_option('mid_layers(default: 3 layers)', list(range(1, 10)))
        head_layers = utils.input_option('head_layers(default: 10 layers)', list(range(1, 18)))
    ...
    def baseline():
        ...
        trainer = block.train.standard.Trainer(
            dataset=trainset, total_epochs=epochs, work_dir=f'{work_dir}/xnn')
    ...
    def train_xnn(): # 脱敏回流训练
        ...
        # summary(xnn_parts, input_size=(3, 112, 112), device="cpu")
        trainer = block.train.standard.Trainer(
            dataset=trainset, total_epochs=epochs, work_dir=f'{work_dir}/xnn')
        trainer.config_dataloader(batch_size=batch_size, num_workers=num_workers)
        trainer.config_logger(log_interval=30)
