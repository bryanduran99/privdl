...
Discard fisrt Layer-norm in RecNet
'''
import os
import math
import random
...
class XNN_Single_Lit(block.model.light.ModelLight):
    ...
    def __init__(self, class_num, isobf=True, pretrained_path='', split_layer=6, tail_layer=6, ext_test=False, is_permute=True, is_matrix=True):
        ...
        self.tail = block.model.moco_v3_vits.vit_base_tail_no_pre_norm(layer=self.tail_layer)
...
def main():
    ...
    result_path = f'{result_path}/{proc_count}_gpus/{train_dataset}/{FLAGS.subset_ratio}_trainset/{FLAGS.obf}_obf/{FLAGS.pretrained != "None" }_pretrained/{FLAGS.split_layer}_layer/{FLAGS.tail_layer}_tail_layer'
    print(result_path)
