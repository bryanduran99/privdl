                ...
                if  (clock.epoch + 1) % 1 == 0 and (clock.epoch + 1) > 1: # 不要每个epoch 都测试，减少测试所需要的时间
                    ...
                    if local_rank == -1 or dist.get_rank() == 0: 
                        ...
                        if True:
                            ...
                            utils.torch_save(ckpt, f'{self.ckpt_dir}'+ '/restart_stage1_tail_vgg_face_epoch_{}.pth'.format(clock.epoch))
                            del ckpt
    ...
    def fit_Adv_Train(self,obf_feature_generator,aux_server_tail,test_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                ...
                if  (clock.epoch + 1) % 1 == 0 and  (clock.epoch + 1) > 3: # 当 epoch 结束时需要执行的操作
                    time.sleep(0.003)
                ...
                if (clock.epoch + 1) % 1 == 0 and (clock.epoch + 1) > 3: # 当 epoch 结束时需要执行的操作
                    ...
                    if local_rank == -1 or dist.get_rank() == 0: 
                        ...
                        if self.current_accuracy > self.best_acc:  # 永远保存最新的
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/attacker_baseline_ckpt.pth')
                            del ckpt_aux
...
def main():
    ...
    parser.add_argument("--train_dataset", default='vggface2', type=str)
    ...
    if FLAGS.mode == 'Visualize':
        ckpt_dir = f'/data/ckpt/NC_VGG_FACE_vis/modeXNN_obf{FLAGS.obf}_{FLAGS.is_permute}_{FLAGS.is_matrix}_{train_dataset}_{attacker_dataset}_8gpu'
    else:
        ckpt_dir = f'/data/ckpt/NC_VGG_FACE/mode_{FLAGS.mode}_obf{FLAGS.obf}_{FLAGS.is_permute}_{FLAGS.is_matrix}_{train_dataset}_{attacker_dataset}_{proc_count}gpu'
    ...
    def train_adv_server():
        ...
        obf_feature_generator = load_checkpoint(obf_feature_generator,'/data/ckpt/NC_VGG_FACE/mode_stage2_obfTrue_True_True_vggface2_facescrub_8gpu/new2_stage2_obf_LN_epoch_18.pth',strict=True)

        test_model = Composed_ModelLit(obf_feature_generator.base_encoder,aux_server_tail)
        # summary(base_encoder.ext, input_size=(3, 224, 224), device="cpu")
        # if local_rank == -1 or dist.get_rank() == 0:
        #     for name, param in model_lit.tail.named_parameters():
        #         print(name, param.shape)
    ...
    def train_disdill():
        ...
        teacher_server_tail = load_checkpoint(teacher_server_tail,'/data/ckpt/NC_VGG_FACE/mode_stage1_obfTrue_True_True_vggface2_facescrub_8gpu/restart_stage1_tail_vgg_face_epoch_9.pth',strict=True)
...
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_VGG_Face.py --mode 'stage1'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_VGG_Face.py --mode 'stage2'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_VGG_Face.py --mode 'stage_aux'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_VGG_Face.py --mode 'stage3'
