...
1. Archtecture purely adopts ViT-B/16， which is trained from scratch.
'''
import os
import math
import random
...
class XNN_Single_Lit(block.model.light.ModelLight):
    ...
    def __init__(self, class_num, isobf=True, pretrained_path='', split_layer=6, ext_test=False):
        super().__init__()
        self.isobf = isobf
        ...
        self.obf = tc.nn.Identity()
        
        self.tail = block.model.moco_v3_vits.vit_base_tail(layer=6)
...
def main():
    ...
    parser.add_argument("--obf", default="False", type=str)
    parser.add_argument("--ext_test", default="False", type=str)
    ...
    FLAGS.ext_test = bool_type(FLAGS.ext_test)
    ...
    result_path = f'{result_path}/{proc_count}_gpus/{train_dataset}/{FLAGS.obf}_obf/{len(FLAGS.pretrained)!=0}_pretrained/{FLAGS.split_layer}_layer'
    print(result_path)
...
if __name__ == '__main__':
    main()
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 032-4.xnn_vit_backbone_vitb16.py --obf False
# rlaunch --charged-group=rface_amp --cpu=8 --gpu=1 --memory=40960 --max-wait-duration=1h -- zsh