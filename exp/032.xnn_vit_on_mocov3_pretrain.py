'''
Implementation slightly adapted from "030.duoka_pretrain_modified_beta.py"
1. XNN train with the pretrained ViT_B/16(the first six layers) by self-supervised(moco V3) as ExtNet.
2. While RecNet is still the 3-layer SimpleViT.
'''
# -*- coding: UTF-8 -*-
import os
import math
...
class XNN_Single_Lit(block.model.light.ModelLight):
    ...
    def __init__(self, class_num, isobf=True, is_identity=False, pretrained_path=''):
        super().__init__()
        self.isobf = isobf
        self.is_identity = is_identity
        self.class_num = class_num

        self.ext = block.model.moco_v3_vits.vit_base_head()
        ...
        self.obf = Obfuscate(197, 768, self.isobf)
        if self.is_identity:
        ...
        self.tail.mode_no_embedding()

        # self.pretrain_ext = tc.nn.Sequential(
        #     self.ext, self.obf, self.tail)
        self.softmax = block.loss.amsoftmax.AMSoftmax(
            in_features=self.tail.feat_size, class_num=self.class_num)
...
def main():
    ...
    result_path = f'{result_path}/{proc_count}_gpus/{train_dataset}/{FLAGS.isobf}_obf/{len(FLAGS.pretrained)!=0}_pretrained'
    print(result_path)
    ...
    def train_xnn_vit(): 
        ...
        # 定义模型
        model_lit = XNN_Single_Lit(class_num=trainset.class_num(), isobf=FLAGS.isobf, is_identity=False, pretrained_path=FLAGS.pretrained)
