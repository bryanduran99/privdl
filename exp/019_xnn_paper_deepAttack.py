...
def main():
    ...
    if obfuscation_type == 'mat':
        ...
        head_layers = 9
        mid_layers = 3
        tail_extend_layers = utils.input_option('tail_extend_layers', list(range(16, 48, 2)))
        result_path = f'{result_path}/headLayers{head_layers}_midLayers{mid_layers}_tailExtendLayers{tail_extend_layers}'
        ern_parts = XnnParts_mid_conv(pretrain_ckpt, head_layers, mid_layers, tail_extend_layers)
    else:
    ...
    def attack_xnn(): # 期望攻击
        model_lit = ERN_lit(ern_parts, class_num=attacker_dataset.class_num())
        trainer = block.train.standard.Trainer(
