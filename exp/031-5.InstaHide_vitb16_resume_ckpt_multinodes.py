...
class InstaTrainer(block.train.standard.Trainer):
    ...
    def config_load_checkpoint(self, is_load=False, local_rank=-1):
        ...
        if self.is_load:
            print(f'config:rank{dist.get_rank()}_torch_load: {self.ckpt_dir}/best_ckpt.pth')
    ...
    def fit_ViT_Insta(self, model_lit, local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                self.scheduler.step()
                if (clock.epoch + 1) % self.test_interval == 0: # 每5个epoch test一次
                    if not (dist.is_available() and dist.is_initialized()) or dist.get_rank() == 0: 
                    ...
                    if dist.is_available() and dist.is_initialized():
                        ...
                        print(f'{dist.get_rank()} entries and waiting test done.......')
                        dist.barrier()
        return model_lit
...
def main():
    ...
    if local_rank != -1:
        ...
        dist.init_process_group(backend='gloo', init_method='env://',world_size=int(os.environ['WORLD_SIZE']), \
                                rank=int(os.environ["RANK"]), timeout=datetime.timedelta(seconds=3600))
    ...
    # print(f'rank{dist.get_rank()}, batch_size:{batch_size}, proc_count:{proc_count}')
    ...
    result_dir = f'{result_dir}/insta/k_{FLAGS.k}/{FLAGS.is_attack}/{train_dataset}/{FLAGS.attacker_dataset}/{proc_count}_gpu'
    print('result_path:', result_dir)
    
    alert_info = f'031-5_k{FLAGS.k}_{FLAGS.is_attack}_{train_dataset}_{FLAGS.attacker_dataset}_{proc_count}gpu'

    ckpt_dir = f'/data/ckpt/insta/031-5_k{FLAGS.k}_{FLAGS.is_attack}_{train_dataset}_{FLAGS.attacker_dataset}_32gpu'
    print('ckpt_dir:', ckpt_dir)
    is_load = False
    if not (dist.is_available() and dist.is_initialized()) or dist.get_rank() == 0:
        if not os.path.exists(ckpt_dir):
            os.makedirs(ckpt_dir)
    if os.path.exists(os.path.join(ckpt_dir, f'best_ckpt.pth')):
        is_load = True
    # is_load = False # debug
    ...
    def train_xnn():  
        ...
        if is_load:
            print(f'rank{dist.get_rank()}_torch_load: {ckpt_dir}/best_ckpt.pth')
        ...
        # DDP
        if dist.is_available() and dist.is_initialized():
            device_id = int(os.environ["RANK"]) % tc.cuda.device_count()
            model_lit = model_lit.to(device_id)

            model_lit = DDP(model_lit, device_ids=[device_id], output_device=device_id)
        else:
        ...
        T_max = 300 # warm_up + cosAnnealLR 的周期
        lr_max = FLAGS.lr
        lr_min = FLAGS.lr_min
        if FLAGS.schedule:
            lambda_lr = lambda cur_iter: max(lr_min/lr_max, cur_iter/warm_up_iter) if cur_iter < warm_up_iter else \
...
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 031-5.InstaHide_vitb16_resume_ckpt_multinodes.py  