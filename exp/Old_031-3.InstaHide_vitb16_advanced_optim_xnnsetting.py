...
class InstaTrainer(block.train.standard.Trainer):
    ...
    def fit_ViT_Insta(self, model_lit, local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                self.scheduler.step()
                if (clock.epoch + 1) % 5 == 0: # 每5个epoch test一次
                    ...
                    if local_rank == -1 or dist.get_rank() == 0: 
                        ...
                        if self.current_accuracy > self.best_acc:
                            ...
                            utils.torch_save(best_model_state_dict, os.path.join(self.ckpt_dir, 'best_model.pth'))
                    if local_rank != -1:
...
def main():
    ...
    result_dir = f'{result_dir}/insta/{train_dataset}/k_{FLAGS.k}/{FLAGS.schedule}LRSche/{proc_count}_gpu'
    ckpt_dir = f'/data/ckpt/insta/031-3/{train_dataset}/k_{FLAGS.k}/{FLAGS.schedule}LRSche/{proc_count}_gpu'
    if local_rank == -1 or dist.get_rank() == 0:
        if not os.path.exists(ckpt_dir):
            os.makedirs(ckpt_dir)
    print('result_path:', result_dir)
    alert_info = f'031-3_k{FLAGS.k}_{FLAGS.is_attack}attack_{FLAGS.train_dataset}_{FLAGS.attacker_dataset}_{proc_count}gpu'
    ...
    def train_xnn():  # 脱敏回流训练
        ...
        # if FLAGS.train_dataset == 'vggface2': # vggface2使用大学习率过拟合，尝试全程使用1e-3
            # lambda_lr = lambda cur_iter: 1e-3/FLAGS.lr
        scheduler = tc.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_lr)

        trainer = InstaTrainer(
            dataset=trainset, total_epochs=train_epoch, scheduler=scheduler, work_dir=result_dir, ckpt_dir=ckpt_dir, k=FLAGS.k, sample_transform=sample_transform, optimizer=optimizer, monitor=monitor)
    ...
    try:
        ...
        print("Exception:", e)
        if local_rank == -1 or dist.get_rank() == 0:
            requests.get(f'https://api.day.app/ocZjTT3y2AhsowCaiKtqLC/break_down_031-3/{alert_info}_{time.strftime("%y%m%d%H%M%S",time.localtime())}')

if __name__ == '__main__':
    main()
    # DDP: 使用torch.distributed.launch启动DDP模式; 使用CUDA_VISIBLE_DEVICES，来决定使用哪些GPU
    # CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 031-3.InstaHide_vitb16_advanced_optim_xnnsetting.py
    # CUDA_VISIBLE_DEVICES="0,1,2,3" python3 -m torch.distributed.launch --nproc_per_node 4 031-3.InstaHide_vitb16_advanced_optim_xnnsetting.py --train_dataset celeba --k 2 --lr 0.5