                ...
                if (clock.epoch + 1) % 2 == 0 and (clock.epoch + 1) > 1: # 当 epoch 结束时需要执行的操作
                    ...
                    if local_rank == -1 or dist.get_rank() == 0: 
                        ...
                        if self.current_accuracy > self.best_acc:  # 永远保存最新的
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/attacker_facescrub_ckpt.pth')
                            del ckpt_aux
...
def main():
    ...
    if FLAGS.mode == 'Visualize':
        _, testset_by_person = block.dataset.hubble.hubble_utils.split_dataset_by_person(
            train_dataset, test_id_num=8, test_img_per_id=125)
    ...
    def plot_embedding(data, label, title,save_name):
        ...
        colors = ['#8ECFC9', '#FFBE7A' , '#FA7F6F' ,'#82B0D2' ,'#BEB8DC' ,'#E7DAD2','#F27970','#C76DA2','#8983BF']
        for i, which_label in enumerate(unique_labels):
            data_i =  data[label == which_label]
            str_label = 'ID_' + str(which_label + 1)
            # label_i = label[label == which_label]
            plt.scatter(data_i[:, 0], data_i[:, 1], marker='o',color=plt.cm.Set1(i),label=str_label,s = 13, alpha=0.7)


        # for i in range(data.shape[0]):
        #     plt.scatter(data[i, 0], data[i, 1], marker='o',color=plt.cm.Set1(label[i] / 8.),label=label[i],s = 3, alpha=0.5)
    ...
    def Visualize():
        ...
        model_lit2 = Composed_ModelLit(obf_feature_generator,server_tail2,trainset.class_num())
        for p in model_lit.parameters():
            p.requires_grad = False
        
        device = tc.device('cuda' if tc.cuda.is_available else 'cpu')
        model_lit = model_lit.to(device)
        model_lit2 = model_lit2.to(device)
        ...
        feats2, labels2 = testers[0].test(model_lit2, vis=True)
        # vis_emb_data ={
        #     'feats': feats,
        #     'labels': labels
        # }
        ...
        vis_data1 = TSNE(n_components=2,perplexity=2, init='pca', random_state=0).fit_transform(feats) 
        vis_data2 = TSNE(n_components=2,perplexity=2, init='pca', random_state=0).fit_transform(feats2) 
        plot_embedding(vis_data1, labels, 't-SNE embedding of faces(baseline)',save_name= '/data/research/version2/privdl/exp/visualization/baseline-tsne.png')
        plot_embedding(vis_data2, labels2, 't-SNE embedding of faces(XNN-d)',save_name= '/data/research/version2/privdl/exp/visualization/xnnd-tsne.png')
        print('done')
        return
