    ...
    def fit_ViT(self, composed_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end(): # 当 epoch 结束时需要执行的操作
                ...
                if  (clock.epoch + 1) % 1 == 0: # 不要每个epoch 都测试，减少测试所需要的时间
                    time.sleep(0.003)
                    ...
                    if local_rank == -1 or dist.get_rank() == 0: 
                        ...
                        if self.current_accuracy > self.best_acc:
                            ...
                            utils.torch_save(ckpt, f'{self.ckpt_dir}/stage1_tail.pth')
                            del ckpt
                            ...
                            utils.torch_save(ckpt, f'{self.ckpt_dir}/stage1_encoder.pth')
                            del ckpt
    ...
    def fit_train_adv_tail(self,obf_feature_generator,aux_server_tail,test_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                ...
                if (clock.epoch + 1) % 1 == 0 and (clock.epoch + 1) > 45: # 当 epoch 结束时需要执行的操作
                    time.sleep(0.006)
                    # 当没有初始化多线程时，因为短路原则，也不会因为get_rank而报错
                    if local_rank == -1 or dist.get_rank() == 0: 
                        self.call_testers(clock, test_model) 
                        ...
                        if self.current_accuracy > self.best_acc:  # 永远保存最新的
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/attack_baseline_facescrub_ckpt.pth')
                            del ckpt_aux
...
def main():
    ...
    if FLAGS.mode == 'Visualize':
        ckpt_dir = f'/data/ckpt/NC3_vis/modeXNN_obf{FLAGS.obf}_{FLAGS.is_permute}_{FLAGS.is_matrix}_{train_dataset}_{attacker_dataset}_8gpu'
    else:
    ...
    if FLAGS.mode == 'Visualize':
        _, testset_by_person = block.dataset.hubble.hubble_utils.split_dataset_by_person(
            train_dataset, test_id_num=8, test_img_per_id=30)
    ...
    def train_disdill():
        ...
        teacher_server_tail = load_checkpoint(teacher_server_tail,'/data/ckpt/NC3/mode_stage1_obfTrue_True_True_celeba_facescrub_8gpu/stage1_tail.pth',strict=True)
    ...
    def plot_embedding(data, label, title):
        ...
        fig.savefig('/data/research/version2/privdl/exp/visualization/facescrub-tsne.png')
        plt.close(fig)
    ...
    def Visualize():
        ...
        server_tail = load_checkpoint(server_tail,'/data/ckpt/NC3/mode_stage1_obfTrue_True_True_celeba_facescrub_8gpu/stage1_tail.pth',strict= True)
        ...
        model_lit = Composed_ModelLit(obf_feature_generator,server_tail,trainset.class_num())
        ...
        vis_data = TSNE(n_components=2,perplexity=2, init='pca', random_state=0).fit_transform(feats) 
        plot_embedding(vis_data, labels, 't-SNE embedding of faces')
        print('done')
        return
...
if __name__ == '__main__':
    # checkpoint = tc.load('/data/ckpt/NC3/mode_stage2_obfTrue_True_True_celeba_facescrub_8gpu/new_stage2_obf_LN_epoch_29.pth', map_location="cpu")    # encoder LN 不更新， noise encoder 更新
    # checkpoint2 = tc.load('/data/ckpt/NC3/mode_stage2_obfTrue_True_True_celeba_facescrub_8gpu/new2_stage2_obf_LN_epoch_29.pth', map_location="cpu")  # 缺失 encoder 的norm
    # checkpoint3 = tc.load('/data/ckpt/NC3/mode_stage2_obfTrue_True_True_celeba_facescrub_8gpu/stage2_obf_LN_epoch_29.pth', map_location="cpu")  #放在obf 中两个norm 都更新


    # checkpoint = tc.load('/data/ckpt/NC_MSRA/mode_stage1_obfTrue_True_True_msra_imdb_16gpu/restart_stage1_tail_epoch_7.pth', map_location="cpu")    # encoder LN 更新， noise encoder 不更新
    checkpoint1 = tc.load('/data/ckpt/NC_VGG_FACE/mode_stage1_obfTrue_True_True_vggface2_facescrub_8gpu/restart_stage1_encoder_epoch_9.pth', map_location="cpu")  # 缺失 encoder 的norm
    # checkpoint2 = tc.load('/data/ckpt/NC_VGG_FACE/mode_stage2_obfTrue_True_True_vggface2_facescrub_8gpu/new2_stage2_obf_LN_epoch_17.pth', map_location="cpu")    # encoder LN 不更新， noise encoder 更新
    # checkpoint3 = tc.load('/data/ckpt/NC_VGG_FACE/mode_stage2_obfTrue_True_True_vggface2_facescrub_8gpu/new2_stage2_obf_LN_epoch_18.pth', map_location="cpu")    # encoder LN 不更新， noise encoder 更新

    main()

# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 300-final-add.py --mode 'stage1'
