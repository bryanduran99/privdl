...
def loss_fn_kd(outputs, labels, teacher_outputs):
    ...
    alpha = 0.9
    T = 20
    KD_loss = tc.nn.KLDivLoss(reduction='batchmean')(tc.nn.functional.log_softmax(outputs/T, dim=1),
                             tc.nn.functional.softmax(teacher_outputs/T, dim=1)) * (alpha * T * T)
            ...
            if clock.epoch_end() and (clock.epoch + 1) % 1 == 0: # 不要每个epoch 都测试，减少测试所需要的时间
                ...
                if local_rank == -1 or dist.get_rank() == 0: 
                    ...
                    if self.current_accuracy > self.best_acc:
                        ...
                        utils.torch_save(ckpt, f'{self.ckpt_dir}/best_ckpt_server_tail.pth')
                        del ckpt
