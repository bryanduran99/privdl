...
B @ MH_n @ 1*1 conv @ MT_(18-n)；n = 1, 2, …, 17
在 中间层特征上随机卷积 混淆是 安全 且 可学性好 的
...
def main():
    ...
    head_layers = utils.input_option('head_layers',
        list(range(1, 18)))
    work_dir = f'{result_path}/head_layers_{head_layers}'
    # 配置数据集 和 相应的预训练模型
    ...
    xnn_parts = XnnParts(pretrain_ckpt, head_layers)
