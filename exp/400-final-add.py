...
400 版本是backbone 加入FE 的 LN 参数更新， 初始化为0,1
'''
import time
import datetime
import os
    ...
    def fit_ViT(self, composed_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end(): # 当 epoch 结束时需要执行的操作
                ...
                if  (clock.epoch + 1) % 1 == 0 and (clock.epoch + 1) > 4: # 不要每个epoch 都测试，减少测试所需要的时间
                    time.sleep(0.003)
    ...
    def fit_train_adv_tail(self,obf_feature_generator,aux_server_tail,test_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                ...
                if (clock.epoch + 1) % 1 == 0 and (clock.epoch + 1) > 5 : # 当 epoch 结束时需要执行的操作
                    time.sleep(0.006)
                    # 当没有初始化多线程时，因为短路原则，也不会因为get_rank而报错
                    if local_rank == -1 or dist.get_rank() == 0: 
                        self.call_testers(clock, test_model) 
                        ...
                        if self.current_accuracy > self.best_acc:  # 永远保存最新的
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/ignore2_ckpt.pth')
                            del ckpt_aux
...
def main():
    ...
    if FLAGS.mode == 'Visualize':
        ...
        ckpt_dir = f'/data/ckpt/NC4/mode_{FLAGS.mode}_obf{FLAGS.obf}_{FLAGS.is_permute}_{FLAGS.is_matrix}_{train_dataset}_{attacker_dataset}_{proc_count}gpu'
    ...
    def train_server_tail_vit(): 
        ...
        base_encoder = Face_adv_encoder(class_num=trainset.class_num(), isobf=FLAGS.obf, \
            pretrained_path=FLAGS.pretrained, split_layer=FLAGS.split_layer, tail_layer=FLAGS.tail_layer, is_permute=FLAGS.is_permute, is_matrix=FLAGS.is_matrix, debug_pos=(FLAGS.debug=='celeba_pos'),is_fix=True, fix_norm= False)
    ...
    def train_adv_vit(): 
        ...
        base_encoder = load_checkpoint(base_encoder,'/data/ckpt/NC4/mode_stage1_obfTrue_True_True_celeba_facescrub_8gpu/stage1_encoder.pth',strict= True)
    ...
    def train_adv_server():
        ...
        # obf_feature_generator = load_checkpoint(obf_feature_generator,'/data/ckpt/NC4/mode_stage2_obfTrue_True_True_celeba_facescrub_8gpu/new2_stage2_obf_LN_epoch_209.pth',strict=True)

        test_model = Composed_ModelLit(base_encoder,aux_server_tail)
        # summary(base_encoder.ext, input_size=(3, 224, 224), device="cpu")
        # if local_rank == -1 or dist.get_rank() == 0:
        #     for name, param in model_lit.tail.named_parameters():
        #         print(name, param.shape)
    ...
    def train_disdill():
        ...
        teacher_server_tail = load_checkpoint(teacher_server_tail,'/data/ckpt/NC4/mode_stage1_obfTrue_True_True_celeba_facescrub_8gpu/stage1_tail.pth')
...
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 400-final-add.py --mode 'stage1'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 400-final-add.py --mode 'stage2'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 400-final-add.py --mode 'stage_aux'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 400-final-add.py --mode 'stage3'