# -*- coding: UTF-8 -*-
'''
train batch和dist batch采用同一个dataloader，mixup+signflip发生在数据集的读取阶段
优点是：
1. 代码简单，无需嵌套的dataloader,预计少踩很多DDP的坑
2. 无需每次train batch都定义dataloader，省时
note:
...
class InstaTrainer(block.train.standard.Trainer):
    ...
    def fit_DDP(self, model_lit, local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            # mixup dataset internal
            # return batch data size is tmp_batch_size // k
