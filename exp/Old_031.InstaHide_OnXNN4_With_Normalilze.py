...
class XNN_Single_Lit(block.model.light.ModelLight):
    ...
    def __init__(self, xnn_parts, class_num, lr, k=4, param_path=None, normalize=None):
        super().__init__()
        self.k = k
        self.lr = lr
...
def main():
    ...
    def train_xnn():  # 脱敏回流训练
        model_lit = XNN_Single_Lit(vit, class_num=trainset.class_num(), lr=FLAGS.lr, k=FLAGS.k, param_path=param_path, normalize=normalize)
        if local_rank != -1:
