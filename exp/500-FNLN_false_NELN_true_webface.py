    ...
    def fit_train_adv_tail(self,obf_feature_generator,aux_server_tail,test_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                ...
                if (clock.epoch + 1) % 2 == 0 and (clock.epoch + 1) > 5: # 当 epoch 结束时需要执行的操作
                    time.sleep(0.006)
                    # 当没有初始化多线程时，因为短路原则，也不会因为get_rank而报错
                    if local_rank == -1 or dist.get_rank() == 0: 
                        self.call_testers(clock, test_model) 
                        ...
                        if self.current_accuracy > self.best_acc:  # 永远保存最新的
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/attacker_imdb_ckpt.pth')
                            del ckpt_aux
    ...
    def fit_train_aux_tail(self,obf_feature_generator,aux_server_tail,teacher_server_tail,test_model,local_rank=-1):
        ...
        for batch_data in clock: # 训练循环
            ...
            if clock.epoch_end():
                ...
                if clock.epoch  % 1 == 0 and clock.epoch > 47: # 当 epoch 结束时需要执行的操作
                    time.sleep(0.003)
                    # 当没有初始化多线程时，因为短路原则，也不会因为get_rank而报错
                    if local_rank == -1 or dist.get_rank() == 0: 
                        self.call_testers(clock, test_model) 
                        ...
                        if self.current_accuracy > self.best_acc:  
                            ...
                            utils.torch_save(ckpt_aux, f'{self.ckpt_dir}/55_10_tail_test_56_epoch.pth')
                            del ckpt_aux
...
def main():
    ...
    parser.add_argument("--train_dataset", default='webface', type=str)
    ...
    if FLAGS.mode == 'Visualize':
        ...
        ckpt_dir = f'/data/ckpt/NC500_webface/mode_{FLAGS.mode}_obf{FLAGS.obf}_{FLAGS.is_permute}_{FLAGS.is_matrix}_{train_dataset}_{attacker_dataset}_{proc_count}gpu'
    ...
    def train_adv_server():
        ...
        obf_feature_generator = load_checkpoint(obf_feature_generator,'/data/ckpt/NC500_webface/mode_stage2_obfTrue_True_True_webface_facescrub_8gpu/new2_stage2_obf_LN_epoch_59.pth',strict=True)
    ...
    def train_disdill():
        ...
        teacher_server_tail = load_checkpoint(teacher_server_tail,'/data/ckpt/NC500_webface/mode_stage1_obfTrue_True_True_webface_facescrub_8gpu/restart_stage1_tail_epoch_6.pth',strict=True)
...
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_webface.py --mode 'stage1'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_webface.py --mode 'stage2'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_webface.py --mode 'stage_aux'
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 500-FNLN_false_NELN_true_webface.py --mode 'stage3'