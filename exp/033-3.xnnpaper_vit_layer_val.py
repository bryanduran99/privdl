...
def main():
    ...
    result_path = f'{result_path}/{FLAGS.mode}_{FLAGS.obf}_{FLAGS.split_layer}_layer_{FLAGS.tail_layer}'
    print(result_path)
    work_dir = result_path
    
    alert_info = f'033-3_{FLAGS.mode}_{FLAGS.obf}_{FLAGS.split_layer}_layer_{FLAGS.tail_layer}'
    tb_log_dir = f'/data/privdl_data/log/{script_file_name}/{FLAGS.split_layer}_layer_{FLAGS.tail_layer}'
    print(tb_log_dir)

    if FLAGS.mode == 'Visualize':
        ckpt_dir = f'/data/ckpt/XNN/XNN_{FLAGS.obf}_{FLAGS.split_layer}_layer_{FLAGS.tail_layer}'
    else:
        ckpt_dir = f'/data/ckpt/XNN/{FLAGS.mode}_{FLAGS.obf}_{FLAGS.split_layer}_layer_{FLAGS.tail_layer}'
    ...
    if FLAGS.mode == 'Visualize':
        _, testset_by_person = block.dataset.hubble.hubble_utils.split_dataset_by_person(
            train_dataset, test_id_num=4, test_img_per_id=30)
...
if __name__ == '__main__':
    main()
# CUDA_VISIBLE_DEVICES="0,1,2,3,4,5,6,7" python3 -m torch.distributed.launch --nproc_per_node 8 033-3.xnnpaper_vit_layer_val.py --obf False --tail_layer 6