...
2. network archetecture:
    following the impl of NeuraCrypt_Repo(conv as encoder and vit as clf), except that mlp_head -> softmax_head
'''
# -*- coding: UTF-8 -*-
import math
import os
...
class NC_Single_Lit(block.model.light.ModelLight):
    ...
    def __init__(self, xnn_parts, class_num):
        ...
        self.pretrain_ext = tc.nn.Sequential(
            xnn_parts.extractor(), xnn_parts.obfuscate(), xnn_parts.tail())
        self.softmax = block.loss.amsoftmax.AMSoftmax(
            in_features=xnn_parts.tail().feat_size, class_num=class_num)

    def forward(self, images, labels, is_inference=False):
        rec_feats = self.pretrain_ext(images)
        if is_inference:   
            return rec_feats    
...
class NC_Parts:
    ...
    def extractor(self):
        # channel_dilation = 2, then encoder's params == 8.6M ~= 8.8M based vit
        return block.model.ViT.Encoder(depth=self.depth, img_size=112, kernel_size=14, in_channels=3, hidden_channels=512, channel_dilation=2, private=True)

    def obfuscate(self):
        '''return a identity() module'''
        return self.obf

    def tail(self):
        model = simpleViT_NC.SimpleViT(
            image_size=128,
            patch_size=16,
            dim=512,
            depth=3,
...
def main():
    ...
    parser.add_argument("--train_dataset", default='facescrub', type=str)
    ...
    result_path = f'{result_path}/{train_dataset}/{proc_count}_gpus/{FLAGS.depth}'
    work_dir = result_path
    
    if train_dataset == 'facescrub':
    ...
    def train_vit_ext(): 
        ...
        # 定义模型
        xnn_parts = NC_Parts(depth=FLAGS.depth)
        model_lit = NC_Single_Lit(xnn_parts, class_num=trainset.class_num())
        ...
        optimizer = tc.optim.SGD(model_lit.parameters(), lr=0.05)
...
if __name__ == '__main__':
    main()
# CUDA_VISIBLE_DEVICES="0,1" python3 -m torch.distributed.launch --nproc_per_node 2 030.duoka_pretrain_modified_beta.py