...
class XnnParts_mid_conv:
    ...
    def __init__(self, pretrain_ckpt, head_layers):
        self.pretrain_ckpt = pretrain_ckpt
        self.head_layers = head_layers

    def extractor(self):
        return block.model.mobile_face_net.MobileFaceNetHead(
            layer_num=self.head_layers)

    def obfuscate(self):
        channels = 64 if self.head_layers < 8 else 128
        return tc.nn.Conv2d(channels, channels, 1, 1, 0, bias=False)
...
def main():
    ...
    obfuscation_type = utils.input_option(
        'obfuscation_type', ['mat', 'mat_full', 'img_conv', 'mid_conv'])
    result_path = f'{result_path}/{obfuscation_type}'

    if obfuscation_type == 'mat':
        ...
        result_path = f'{result_path}/head_layers_{head_layers}'
    ...
    for tester in testers:
        tester.config_dataloader(batch_size=batch_size)
    # 预训练模型测试
    ...
    def train_xnn(): # 脱敏回流训练
        ...
        trainer.config_dataloader(batch_size=batch_size)
        trainer.config_tester(testers, interval=5*60)
        trainer.fit(model_lit)
    train_xnn()

    def attack_xnn(): # 期望攻击
        model_lit = INN_Lit(xnn_parts, pretrain_ckpt)
        trainer = block.train.standard.Trainer(
            dataset=attacker_dataset, total_epochs=15, work_dir=f'{work_dir}/inn')
        RIA = block.test.top1_test.RestoreIdentificationAccuracy(
            dataset=testset_by_person, name='RestoreIdentificationAccuracy')
        RIA.config_dataloader(batch_size=batch_size)
        trainer.config_dataloader(batch_size=batch_size)
