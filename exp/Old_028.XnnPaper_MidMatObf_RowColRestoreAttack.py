...
def recover_feats(feats):
    B, _, _, C = feats.shape
    for i in range(B):
        # feature map per image
        feat = rearrange(feats[i], 'h w c -> c h w')
        for j in range(C):
            # one channel of feature map
            feat[j] = recover_rows(feat[j])
            feat[j] = recover_rows(rearrange(feat[j], 'h w -> w h')) 
            feat[j] = rearrange(feat[j], 'w h -> h w')
        feats[i] = rearrange(feat, 'c h w -> h w c')
    return feats
...
def recover_rows(x):
    ...
    rst = tc.tensor(rst)  # show_image(np.array(rst))
    return rst
...
class ERN_lit(XNN_Single_Lit):
    ...
    def train_step(self, batch_data):
        ...
        with tc.no_grad():
            mid_feats = self.extractor(imgs)
            mid_feats = self.obfuscate(mid_feats)
        mid_feats = recover_feats(mid_feats) ### restore feature map 
        scores = self(mid_feats, labels)
        loss = tc.nn.functional.cross_entropy(scores, labels)
        utils.step(self.optimizer, loss)
    ...
    def inference(self, batch_input):
        mid_feats = self.extractor(batch_input)
        mid_feats = self.obfuscate(mid_feats)
        mid_feats = recover_feats(mid_feats) ###
        return self.tail(mid_feats)
...
def main():
    ...
    obfuscation_type = utils.input_option(
        'obfuscation_type', ['no', 'mat', 'mat_full', 'img_conv', 'mid_conv', 'mid_mat'])
    ...
    if obfuscation_type == 'no':
        ...
        xnn_parts_ERN = XnnParts_mid_mat(pretrain_ckpt, head_layers, 'nonlinear', tail_extend_layers=head_layers)
    else:
    ...
    def attack_xnn(): # 期望攻击
        ...
        trainer.config_dataloader(batch_size=batch_size, num_workers=num_workers)
        trainer.config_tester([RIA], interval=60*60)
        trainer.fit(model_lit)
    attack_xnn()
